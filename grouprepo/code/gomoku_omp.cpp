...
move_t* getPossibleMoves(playedMoves_t played, int* sum){
	// Make a temporary board, add stones and get possible moves
	Board tempBoard;
...
/*
 * Using the shapreScore function, calculate the score for the player 'evaluateFor' and at row 'rowNum' given that the current player is 'currentPlayer' at a diagonal
 */
...
score_t calculateBoardScoreTotal(playedMoves_t played, int currentPlayer){
	...
	// Score for current player and other player
...
/*
 * Minimax function without alpha-beta pruning from the initial board with played moves 'played'
 */
...
/*
 * Get best move from the current board state using OpenMP to delegate tasks to multiple threads
 */
move_t getBestMoveAB(int depth, bool maximizingPlayer, int currentPlayer){
	...
	// Use omp parallel for loop for each possible move
